...
namespace Worker
{
	public class Worker : BackgroundService
	{
		...
		private readonly Object objectKey = new { };
						...
						using (var consumer = new ConsumerWrapper<Null, string>(consumerConfig, topic))
						{
							while (true)
							{
								try
								{
									...
									var a = DatabaseConnector._sqlConnection?.State;
									Console.WriteLine(a);
									var exam = JsonConvert.DeserializeObject<Exam>(message);
									Console.WriteLine(exam.Result);

									var oldExam = await _examRepo.GetEntityByIdAsync(exam.ExamId);

									Console.WriteLine(oldExam.Result);
									if (oldExam.Status != 1)
									{
